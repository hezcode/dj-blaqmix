  ...
  const statusLabel =
    event.status === "sold-out"
      ...
      : event.status === "cancelled"
        ...
        : "Upcoming";
  const canBuyTicket =
    event.status === "upcoming" && Boolean(event.pricingTiers?.length);
    ...
    <>
      ...
      >
        <div className="grid min-h-[740px] overflow-hidden rounded-[28px] border border-white/10 bg-[#0b0b0c] shadow-[0_20px_80px_rgba(0,0,0,0.28)] sm:min-h-[480px] sm:grid-cols-[minmax(0,1.65fr)_minmax(240px,0.95fr)]">
          <div className="relative flex min-h-[520px] flex-col justify-between overflow-hidden px-5 py-5 sm:min-h-80 sm:px-7 sm:py-7">
            ...
            <div className="relative z-10 flex items-start justify-between gap-4">
              ...
              {(event.venueName || event.city) && (
                <p className="hidden min-w-[50px] text-right font-poppins text-[11px] uppercase tracking-[0.22em] text-white/45 sm:block sm:min-w-[120px] sm:text-xs">
          ...
          <div className="group relative min-h-[220px] border-t border-white/10 sm:min-h-full sm:border-l sm:border-white/10 sm:border-t-0">
            {event.coverImage ? (
              <Image
            ...
            <div className="absolute bottom-0 left-0 right-0 z-30 bg-linear-to-t from-black via-black/84 to-transparent px-5 pb-5 pt-16 text-right sm:px-6 sm:pb-6">
              {canBuyTicket ? (
                <CustomButton
                  text="Make Table Reservation"
                  onClick={() => setIsTicketModalOpen(true)}
                  className="ml-auto justify-center rounded-2xl bg-blaqmix-red px-5 py-3 text-sm font-semibold text-white transition-all duration-300 hover:bg-blaqmix-red/90"
                />
              ) : null}
            </div>
          </div>
